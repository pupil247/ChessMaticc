...
Effect *EffectFactory::CreateEffect(string effectDefinition, int defaultCount, int defaultDelay)
{
	...
	try
	{
		...
		{
			ESP_LOGI("TEST","CREATE RAINBOW");
			printf("create rainbow");
		...
		{
			effect = CreateStatic(effectParameters, defaultCount, defaultDelay);
		}
		else
		{
			ESP_LOGI("TEST","UNKWNOKN");
...
Effect *EffectFactory::CreateStatic(string parameters, int defaultCount, int defaultDelay)
{
	return (Effect *)new Static(defaultCount, 100);
}
